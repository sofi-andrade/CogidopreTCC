...
function renderizarCards() {
  ...
  const dadosFiltrados = dadosEsportes.filter(item => {
    // Normalização para comparar categorias
    ...
    // Normalização e busca flexível (por título ou local)
    const buscaTratada = termoBusca.toLowerCase().trim();
    const tituloItem = item.titulo.toLowerCase().trim();
    const localItem = item.local.toLowerCase().trim();

    const bateuPesquisa = buscaTratada === "" || 
...
function inicializar() {
  // Renderiza os cards assim que a página carrega
  renderizarCards();
  ...
  // Abertura e filtragem da pesquisa no header
  const searchBox = document.querySelector(".search-box");
  const searchBtn = document.querySelector(".search-btn");
  const searchInput = document.querySelector(".search-input");

  if (searchBtn && searchBox && searchInput) {
    // Abre/Fecha ao clicar no botão da lupa
    ...
    // Filtra os cards em tempo real ao digitar
...
// Garante que o script rode após o DOM carregar completamente
